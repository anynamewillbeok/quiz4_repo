...
public:
  ...
  /**
  Execute or stop controller execution
  @param start true to start execution if goals are set, false to stop execution
  @return bool indicating if command was accepted
  */
  virtual bool execute(bool start) = 0;


  /**
  Returns platform status (indicating if it is executing a series of goals or idle - waiting for goals)
  @param[in|out] currentGoalIndex The index of the current goal being pursued (0-based)
  @return platform status
  */
  virtual pfms::PlatformStatus status(unsigned int& currentGoalIndex) = 0;


  /**
  Setter for goals
  @param goals vector of goals to be pursued by platform, in order supplied, any previous goals should be cleared
  ...
  */
  ...
  /**
  Set tolerance when reaching goal or obstacle
  ...
  */
  ...
  /**
  returns current obstacles information
  @return vector of points representing obstacles in the environment
  */
